...
```bash
docker run -it --rm -p 8800:8800 -p 8765:8765 -p 8766:8766 ghcr.io/xs-mlvp/workshop:latest
```
...
1. **Configure your credentials**  
   Edit the provided `env.template` file and fill in your API credentials (e.g., OpenAI or a compatible interface):
...
2. **Launch with API Access**  
   Run the container, attaching the `env.template` file so the agents can connect to the LLM:
   ```bash
   docker run -it --rm --network host --env-file env.template ghcr.io/xs-mlvp/workshop:latest
   ```
